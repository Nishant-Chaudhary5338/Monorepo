...
export async function readDesignFile(filePath: string, pageNumber = 1): Promise<DesignFileResult> {
  if (!fs.existsSync(filePath)) throw new Error(`File not found: ${filePath}`);
  const ext = path.extname(filePath).toLowerCase();
  if (ext === '.pdf') return readPdfPage(filePath, pageNumber);
  if (IMAGE_EXTS.has(ext)) return readImageFile(filePath);
...
async function readImageFile(filePath: string): Promise<DesignFileResult> {
...
async function readPdfPage(filePath: string, pageNumber: number): Promise<DesignFileResult> {
  ...
  try {
    ...
    const result = spawnSync(
      PDFTOPPM,
      ['-png', '-r', '150', '-f', String(pageNumber), '-l', String(pageNumber), filePath, outPrefix],
      { encoding: 'utf8', timeout: 20000 }
    );

    if (result.error) throw new Error(`pdftoppm failed: ${result.error.message}`);
    if (result.status !== 0) throw new Error(`pdftoppm error: ${result.stderr || 'unknown'}`);

    const files = fs.readdirSync(tmp).filter(f => f.endsWith('.png') || f.endsWith('.ppm'));
    if (files.length === 0) throw new Error('pdftoppm produced no output');

...
class IXDGeneratorServer extends McpServerBase {
    ...
    registerTools() {
        ...
        this.addTool('read_design_file', 'Read a PDF page or image file and return it as an image for Claude/Cline to analyze visually. Use this first, then describe what you see and generate layout code.', {
            type: 'object',
            properties: {
        ...
        this.addTool('get_ui_catalog', 'Scan a @repo/ui (or any shared-ui) components directory and return a full catalog of available components with their props. Use this to know which components are available before generating layout code.', {
            type: 'object',
            properties: {
                components_dir: {
                    type: 'string',
        ...
        this.addTool('write_layout', 'Write the AI-generated React layout code to a file in the target app. Creates parent directories if needed.', {
            type: 'object',
            properties: {
        ...
        this.addTool('list_pdf_pages', 'Get the total number of pages in a PDF file.', {
            type: 'object',
            properties: {
    ...
    async handleReadDesignFile(args) {
        const { file_path, page = 1 } = args;
        try {
    ...
    async handleGetUICatalog(args) {
        const { components_dir, package_name = '@repo/ui' } = args;
        try {
    ...
    async handleWriteLayout(args) {
        const { output_path, code, overwrite = false } = args;
        try {

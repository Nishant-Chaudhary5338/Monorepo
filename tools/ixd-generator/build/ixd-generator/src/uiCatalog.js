...
export function getUICatalog(componentsDir, packageName = '@repo/ui') {
...
function extractPropsPreview(typesFile, mainFile) {
...
export function formatCatalogForPrompt(catalog) {
    ...
    for (const c of catalog.components) {
        ...
        if (c.hasVariants)
            lines.push(`- Has variants (check component for variant prop options)`);
        if (c.propsPreview)
            lines.push(`- Props: ${c.propsPreview}`);
        lines.push('');
    }
    return lines.join('\n');
}
//# sourceMappingURL=uiCatalog.js.map
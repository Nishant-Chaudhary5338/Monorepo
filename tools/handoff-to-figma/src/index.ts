...
interface ScreenEntry {
  /** PascalCase component name, e.g. "HomeScreen" */
  name: string;
  /** Absolute path to the JSX file containing this component */
  file: string;
  /** Extracted function body (may be truncated for large SVG-heavy components) */
  content: string;
}

interface PageEntry {
  /** "Mobile" | "Web" | "Guidelines" | "Merch" */
  name: string;
  screens: ScreenEntry[];
}
...
function findProjectRoot(baseDir: string): string {
  if (fs.existsSync(path.join(baseDir, 'project'))) return baseDir;

  for (const entry of fs.readdirSync(baseDir, { withFileTypes: true })) {
    if (!entry.isDirectory()) continue;
    const candidate = path.join(baseDir, entry.name);
    if (fs.existsSync(path.join(candidate, 'project'))) return candidate;
  }

  throw new Error(
    `Cannot find project/ directory under ${baseDir}. ` +
...
function extractBrandTokens(brandJsx: string): Record<string, string> {
 ...
 */
function extractComponentBody(jsxContent: string, componentName: string): string {
  const startRe = new RegExp(
    `(?:function\\s+${componentName}\\s*\\([^)]*\\)\\s*\\{|const\\s+${componentName}\\s*=.*?=>\\s*\\{)`
  );
...
function truncateIfLarge(content: string, maxBytes = 4096): string {
...
function collectScreensFromFiles(projectDir: string, relFiles: string[], pageName: string): PageEntry | null {
...
function detectPages(projectDir: string): PageEntry[] {
  const pages: PageEntry[] = [];

  // Mobile: screens.jsx + app.jsx at project root
  const mobilePage = collectScreensFromFiles(
    projectDir,
    ['screens.jsx', 'app.jsx'],
    'Mobile'
  );
  if (mobilePage) pages.push(mobilePage);
...
function findAssets(projectDir: string): string[] {
...
class HandoffToFigmaServer extends McpServerBase {
      ...
      ].join(' '),
      {
        ...
        properties: {
          zipPath: {
            type: 'string',
            description:
              'Absolute or ~-prefixed path to the handoff .zip file, ' +
  ...
  private async handleListHandoffs(args: unknown): Promise<ToolResult> {
    const { searchDir } = args as { searchDir?: string };
    const dir = searchDir
  ...
  private async handleParseHandoff(args: unknown): Promise<ToolResult> {
    const { zipPath } = args as { zipPath: string };
    ...
    if (resolved.endsWith('.zip')) {
      try {
        ...
        return this.error(
          `Failed to extract zip at ${resolved}: ${String(err)}. ` +
    ...
    if (fs.existsSync(brandFile)) {
      ...
      if (detected.length > 0) fonts = detected;
    }
    ...
    const result: HandoffParseResult = {
      projectName,
  ...
  private async handleReadScreenContent(args: unknown): Promise<ToolResult> {
    const { filePath, screenName } = args as { filePath: string; screenName: string };
    ...
    if (!body) {
      ...
      return this.error(
        `Component "${screenName}" not found in ${path.basename(resolved)}. ` +

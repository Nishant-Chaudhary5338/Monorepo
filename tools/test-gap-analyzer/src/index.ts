...
function extractExports(content: string): ExportInfo[] {
  ...
  for (const i = 0; i < lines.length; i++) {
    ...
    if (directMatch) {
      ...
      else if (name[0] === name[0].toUpperCase() && name.includes('Component') || line.includes('React')) type = 'component';
      else if (name.startsWith('use') && name[3] === name[3].toUpperCase()) type = 'hook';
...
function analyzeTestFile(testContent: string): { testedFunctions: string[]; testedBehaviors: Map<string, string[]> } {
...
function detectMissingEdgeCases(
  ...
  for (const edgeCase of cases) {
    ...
    const isCovered = testedLower.some(test =>
      caseKeywords.some(keyword => test.includes(keyword))
    );
    if (!isCovered) {
...
function scanDirectory(dir: string, extensions: string[] = ['.ts', '.tsx', '.js', '.jsx']): string[] {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (['node_modules', 'build', 'dist', '.next', '__snapshots__'].includes(entry.name)) continue;
...
class TestGapAnalyzerServer extends McpServerBase {
  ...
  constructor() {
    this.server = new Server(
      { name: 'test-gap-analyzer', version: '1.0.0' },
      { capabilities: { tools: {} } }
    );

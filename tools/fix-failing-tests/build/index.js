...
// MAIN SERVER
// ============================================================================
class FixFailingTestsServer {
    server;
    constructor() {
        this.server = new Server({ name: 'fix-failing-tests', version: '1.0.0' }, { capabilities: { tools: {} } });
    ...
    setupToolHandlers() {
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                {
                    name: 'run_tests',
                    description: 'Run the test suite and collect results including pass/fail counts and failure details',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            projectRoot: { type: 'string', description: 'Root directory of the project' },
                            testPath: { type: 'string', description: 'Specific test file or directory to run (optional)' },
                        },
                ...
                },
                {
                    ...
                    description: 'Analyze test failures, classify error types, and generate fix suggestions',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            projectRoot: { type: 'string', description: 'Root directory of the project' },
                            testPath: { type: 'string', description: 'Specific test file or directory to analyze' },
                        },
                        required: ['projectRoot'],
                    },
                },
                {
                    name: 'auto_fix_tests',
                    description: 'Run tests, analyze failures, and generate auto-fix code for each failure',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            projectRoot: { type: 'string', description: 'Root directory of the project' },
                            testPath: { type: 'string', description: 'Specific test file to fix' },
                        },
                        required: ['projectRoot'],
                    },
                },
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            switch (request.params.name) {
                ...
                case 'analyze_failures':
                    return await this.handleAnalyzeFailures(request.params.arguments);
                case 'auto_fix_tests':
                    return await this.handleAutoFix(request.params.arguments);
                default:
    ...
    async handleAnalyzeFailures(args) {
        ...
        try {
            ...
            if (result.failed === 0) {
                return {
                    content: [{ type: 'text', text: JSON.stringify({ success: true, message: 'All tests passing', passed: result.passed }, null, 2) }],
                };
            }
    ...
    async handleAutoFix(args) {
        ...
        try {
            ...
            if (result.failed === 0) {
                return {
                    content: [{ type: 'text', text: JSON.stringify({ success: true, message: 'All tests passing - no fixes needed', passed: result.passed }, null, 2) }],
                };
            }
    ...
    async run() {
        ...
        console.error('Fix Failing Tests MCP server running on stdio');
    }
}
const server = new FixFailingTestsServer();

...
function detectTestRunner(projectRoot: string): 'vitest' | 'jest' | 'unknown' {
...
function runTests(projectRoot: string, testPath?: string): TestRunResult {
...
function parseTestOutput(output: string, runner: string, duration: number): TestRunResult {
...
function classifyFailure(testName: string, file: string, error: string): TestFailure {
...
function generateFix(failure: TestFailure, sourceContent: string): string {
...
function generateImportFix(failure: TestFailure, sourceContent: string): string {
...
function generateAssertionFix(failure: TestFailure): string {
...
function generateTypeFix(failure: TestFailure): string {
...
function generateTimeoutFix(_failure: TestFailure): string {
...
function generateRuntimeFix(failure: TestFailure, sourceContent: string): string {
...
function findSourceForTest(testFile: string): string | null {
...
class FixFailingTestsServer extends McpServerBase {
  ...
  private async handleAnalyzeFailures(args: unknown) {
    const { projectRoot, testPath } = args;
    try {
      const result = runTests(projectRoot, testPath);
      if (result.failed === 0) {
  ...
  private async handleAutoFix(args: unknown) {
    const { projectRoot, testPath } = args;
    try {
      const result = runTests(projectRoot, testPath);
      if (result.failed === 0) {

...
export async function detectAntiPatterns(appPath, config) {
    ...
    for (const file of files) {
        ...
        if (analysis.hooks.some((h) => h.name === 'useState'))
            responsibilities.add('state');
        if (analysis.hooks.some((h) => h.name === 'useEffect'))
        ...
        if (importSources.some((s) => s.includes('router')))
            responsibilities.add('routing');
        if (importSources.some((s) => s.includes('redux') || s.includes('store')))
            responsibilities.add('store');
        if (analysis.jsxElements.length > 30)

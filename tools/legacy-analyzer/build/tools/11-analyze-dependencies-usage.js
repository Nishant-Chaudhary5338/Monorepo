...
export async function analyzeDependenciesUsage(appPath, config) {
    ...
    for (const dep of Object.keys(allDeps)) {
        if (LIBRARY_PATTERNS[dep])
            continue;
        if (dep.startsWith('@types/') || dep.startsWith('eslint') || dep.startsWith('typescript'))
            continue;
        const usageCount = 0;
        for (const file of files) {
            const content = readFileContent(file);
    ...
    const deepImportList = deepImports.map((d) => `${path.relative(appPath, d.file)}: ${d.import} (depth: ${d.depth})`);
    const crossFeatureList = crossFeatureImports.map((c) => `${path.relative(appPath, c.from)} -> ${path.relative(appPath, c.to)} via ${c.importPath}`);
    // 3. Detect circular dependencies
    const cycles = detectCircularDependencies(graph);
    const circularIssues = cycles.map((cycle) => `Circular: ${cycle.map((f) => path.relative(appPath, f)).join(' -> ')}`);
    ...
    for (const file of files) {
        ...
        const hasUIImport = imports.some((i) => i.source.includes('@repo/ui') || i.source.includes('@monorepo/ui'));
        if (hasUIImport) {
            uiPackageUsage.used = true;
        }
        // Check for reimplemented UI components
        const uiComponentNames = ['Button', 'Input', 'Card', 'Modal', 'Dialog', 'Select', 'Checkbox'];
    ...
    // 5. Check utils usage
    const duplicatedUtils = [];
    const missingCentral = [];
    const utilFunctions = new Map();
    for (const file of files) {
        const content = readFileContent(file);
        if (!content)
            continue;
...
//# sourceMappingURL=11-analyze-dependencies-usage.js.map
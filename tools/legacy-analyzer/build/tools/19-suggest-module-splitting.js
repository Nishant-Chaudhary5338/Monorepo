...
import { isKitchenSinkFile, suggestUtilitySplits, } from '../utils/refactor-helpers.js';
const LARGE_FILE_THRESHOLD_BYTES = 10 * 1024; // 10KB
const LARGE_LINE_THRESHOLD = 200;
const MANY_EXPORTS_THRESHOLD = 10;
export async function suggestModuleSplitting(appPath, config) {
    ...
    for (const file of files) {
        ...
        if (isKitchenSinkFile(file)) {
            ...
            if (suggestedSplits.length >= 2) {
                const proposedFiles = suggestedSplits.map((s) => `${basename}/${s.category}.ts`);
                splits.push({
                    file: relPath,
        ...
        if (lineCount > mergedConfig.largeUtilLines || fileSize > LARGE_FILE_THRESHOLD_BYTES) {
            ...
            if (parsed) {
                ...
                if (exports.length >= MANY_EXPORTS_THRESHOLD) {
                    ...
                    if (groups.length >= 2) {
                        const proposedFiles = groups.map((g) => `${basename}/${g.domain}.ts`);
                        splits.push({
                            file: relPath,
 ...
 */
function groupExportsByDomain(exportNames) {
    const domains = {
        date: [],
        string: [],
        validation: [],
...
//# sourceMappingURL=19-suggest-module-splitting.js.map
...
import type { AnalyzeLegacyAppOutput, MigrationHint, AnalyzerConfig, ToolInput } from '../types';
...
export async function analyzeLegacyApp(appPath: string, config?: Partial<AnalyzerConfig>): Promise<AnalyzeLegacyAppOutput> {
  ...
  if (tech.language === 'JavaScript') healthScore -= 10;
  if (!routing.lazyLoading && routing.routeCount > 5) healthScore -= 5;

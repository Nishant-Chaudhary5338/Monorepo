...
function scanSourceFiles(dir: string, exts: string[] = ['.ts', '.tsx', '.js', '.jsx']): string[] {
  const files: string[] = [];
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (['node_modules', 'build', 'dist', '.next', '__tests__', '.git'].includes(entry.name)) continue;
...
function getExternalPackageName(importPath: string): string | null {
...
class DepAuditorServer extends McpServerBase {
    ...
    for (const pkg of filtered) {
      ...
      for (const file of sourceFiles) {
        ...
        for (const imp of imports) {
          ...
          if (extName) usedDeps.add(extName);
        }
      }

      const unused = [...declaredDeps].filter(d => {
        if (d.startsWith('@types/')) return false; // Type defs are "used" implicitly
        if (['typescript', 'vitest', 'jest', 'eslint', 'prettier', '@types/node'].includes(d)) return false; // Tooling
    ...
    for (const pkg of filtered) {
      ...
      for (const [dep, declaredVersion] of Object.entries(allDeps)) {
        try {
          ...
          const declared = declaredVersion as string;
  ...
  private handleAnalyzeBundleImpact(args: any) {
    ...
    for (const pkg of packages) {
      ...
      for (const file of sourceFiles) {
        ...
        for (const imp of imports) {
          const extName = getExternalPackageName(imp);
          if (extName) usedInSrc.add(extName);
        }
      }
  ...
  private handleDepSizes(args: any) {
    ...
    const depSizes: { name: string; sizeBytes: number; sizeMB: string }[] = [];

...
// MAIN SERVER
// ============================================================================
class QualityPipelineServer {
    server;
    constructor() {
        this.server = new Server({ name: 'quality-pipeline', version: '1.0.0' }, { capabilities: { tools: {} } });
            ...
            tools: [
                {
                    ...
                    description: 'Run the complete quality pipeline: tests → performance → accessibility → design tokens',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            projectRoot: { type: 'string', description: 'Root directory of the project' },
                            skipStages: { type: 'array', items: { type: 'string' }, description: 'Stages to skip (tests, performance, accessibility, design)' },
                        },
                ...
                },
                {
                    ...
                    description: 'Run specific stages of the quality pipeline',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            projectRoot: { type: 'string', description: 'Root directory of the project' },
                            stages: { type: 'array', items: { type: 'string' }, description: 'Stages to run (tests, performance, accessibility, design)' },
                        },
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            switch (request.params.name) {
                case 'run_full_pipeline':
                    return await this.handleFullPipeline(request.params.arguments);
                case 'run_partial_pipeline':
                    return await this.handlePartialPipeline(request.params.arguments);
                default:
    ...
    async run() {
        ...
        console.error('Quality Pipeline MCP server running on stdio');
    }
}
const server = new QualityPipelineServer();

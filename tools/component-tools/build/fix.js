...
function fixImportPaths(content) {
    ...
    if (content.includes('@/lib/utils')) {
        modifiedContent = modifiedContent.replace(/import\s+{?\s*cn\s*}?\s+from\s+["']@\/lib\/utils["']/g, 'import { cn } from "../../lib/utils"');
        fixed.push('Fixed @/lib/utils import path to relative path');
    }
    return { content: modifiedContent, fixed };
}
function fixAccessibility(content, componentName) {
    const fixed = [];
    let modifiedContent = content;
    // Add displayName if missing
    if (!content.includes('.displayName')) {
        const displayNameLine = `${componentName}.displayName = '${componentName}';`;
        modifiedContent = modifiedContent.replace(/export\s+{[^}]*}/, `export { ${componentName} }\n\n${displayNameLine}`);
    ...
    if (!content.includes('aria-')) {
        ...
        if (componentName.toLowerCase().includes('button') || content.includes('onClick')) {
            modifiedContent = modifiedContent.replace(/<div([^>]*onClick[^>]*)>/g, '<div$1 role="button" tabIndex={0}>');
            fixed.push('Added role="button" and tabIndex for accessibility');
        }
    }
    // Add focus-visible styles
    if (!content.includes('focus-visible')) {
        modifiedContent = modifiedContent.replace(/className=\{[^}]*\}/g, (match) => {
    ...
    if (!content.includes('onKeyDown') && content.includes('onClick')) {
        modifiedContent = modifiedContent.replace(/onClick=\{[^}]*\}/g, (match) => {
...
function fixSemanticHtml(content, componentName) {
    const fixed = [];
    let modifiedContent = content;
    // Replace div with button for button components
    if (componentName.toLowerCase().includes('button') && content.includes('<div')) {
        modifiedContent = modifiedContent.replace(/<div([^>]*onClick[^>]*)>/g, '<button$1>');
        modifiedContent = modifiedContent.replace(/<\/div>/g, '</button>');
        fixed.push('Replaced <div> with <button> for proper semantic HTML');
    }
    return { content: modifiedContent, fixed };
}
function fixTypes(content, componentName) {
    const fixed = [];
    let modifiedContent = content;
...
function fixRefForwarding(content, componentName) {
    const fixed = [];
    let modifiedContent = content;
    // Add ref forwarding if missing
    if (!content.includes('forwardRef') && content.includes('React.FC')) {
        modifiedContent = modifiedContent.replace(/export\s+const\s+(\w+):\s*React\.FC/g, 'export const $1 = React.forwardRef<HTMLElement, ${componentName}Props>(');
        modifiedContent = modifiedContent.replace(/}\);?\s*$/m, '});');
        fixed.push('Added ref forwarding for better component composition');
    }
    return { content: modifiedContent, fixed };
}
function fixClassNameMerging(content) {
    const fixed = [];
    let modifiedContent = content;
    // Add cn utility for className merging
    if (!content.includes('cn(') && content.includes('className')) {
        modifiedContent = modifiedContent.replace(/className=\{`([^`]*)`\}/g, 'className={cn(`$1`, className)}');

...
function readWorkspaceConfig(root: string): string[] {
...
function getPackageInfo(pkgPath: string): PackageInfo | null {
  ...
  if (relativePath.includes('/apps/')) type = 'app';
  else if (relativePath.includes('/tools/')) type = 'tool';
  else if (relativePath.includes('/configs/') || relativePath.includes('/packages/eslint') || relativePath.includes('/packages/typescript') || relativePath.includes('/packages/tailwind')) type = 'config';
...
function getWorkspaceInfo(root: string): WorkspaceInfo {
...
function getDependencyGraph(packages: PackageInfo[]): Map<string, string[]> {
...
function findDependents(packages: PackageInfo[], targetPkg: string): string[] {

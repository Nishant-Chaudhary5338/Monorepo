...
class McpToolImproviserServer extends McpServerBase {
    ...
    registerTools() {
        this.addTool('analyze_tool', 'Deep analysis of a single MCP tool across 7 dimensions (description quality, schema completeness, error handling, edge case coverage, response structure, code quality, contextual depth). Returns JSON report with scores, detailed issues, and proposed diffs with reasons explaining each improvement.', {
            type: 'object',
            properties: {
                path: {
                    type: 'string',
        ...
        this.addTool('batch_analyze', 'Analyze ALL MCP tools in the tools/ directory and return a comprehensive report. Shows worst/best performers, summary statistics, and all proposed diffs.', {
            type: 'object',
            properties: {
                toolsDir: {
                    type: 'string',
        ...
        this.addTool('apply_improvements', 'Apply approved diffs to improve MCP tools. Creates timestamped backups (.bak files) before modifying.', {
            type: 'object',
            properties: {
                diffs: {
                    type: 'array',
        ...
        this.addTool('rollback', 'Restore a file from its backup. Use this if an applied improvement caused issues.', {
            type: 'object',
            properties: {
    ...
    async handleAnalyzeTool(args) {
        ...
        const result = analyzeTool(resolvedPath);
    ...
    async handleBatchAnalyze(args) {
        const startTime = Date.now();
        const { toolsDir: inputToolsDir } = (args || {});

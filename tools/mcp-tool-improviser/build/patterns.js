...
export const descriptionPatterns = {
    ...
    enrichDescription(toolName, currentDesc, inputSchema) {
        const changes = [];
        ...
        const optional = Object.keys(props).filter((p) => !required.includes(p) && props[p].description);
        const optionalNote = optional.length > 0
            ? ` Optional: ${optional.map((p) => `${p} (${props[p].description})`).join(', ')}.`
            : '';
...
export const schemaPatterns = {
    checkProperty(propName, prop) {
        ...
        if (prop.type === 'string' && !prop.enum && !prop.pattern && !prop.format) {
        ...
        if (prop.type === 'number' && (propName.includes('threshold') || propName.includes('limit') || propName.includes('max'))) {
    ...
    checkRequiredVsDefault(schema) {
        const issues = [];
        const required = schema.required || [];
        const properties = schema.properties || {};
        for (const [propName, prop] of Object.entries(properties)) {
            if (!required.includes(propName) && prop.default === undefined && propName !== 'root') {
                issues.push({
                    dimension: 'schemaCompleteness',
                    severity: 'low',
                    location: `inputSchema.${propName}`,
...
export const errorPatterns = {
    ...
    checkGenericErrorHandling(source) {
    ...
    generateStructuredError(errorVar = 'error') {
        return `{
      content: [{
        type: 'text',
        text: JSON.stringify({
    ...
    checkMissingValidation(source, toolName) {
        const issues = [];
...
export const responsePatterns = {
    ...
    checkMissingSuccessField(source) {
...
export const codeQualityPatterns = {
    checkDuplicatedScanDirectory(source, filePath) {
    ...
    checkDuplicatedFindTsconfig(source, filePath) {
...
export const contextualPatterns = {
    ...
    checkMissingConfidenceScores(source) {
    ...
    checkMissingSuggestions(source) {
...
// ============================================================================
export function runAllPatternChecks(source, filePath, tools) {
    const issues = [];
    ...
    for (const tool of tools) {
        ...
        if (tool.inputSchema) {
            issues.push(...schemaPatterns.checkRequiredVsDefault(tool.inputSchema));
        }
    }
    // Code-level checks

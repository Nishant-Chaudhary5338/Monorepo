...
interface TokenViolation {
  type: 'hardcoded-color' | 'hardcoded-spacing' | 'hardcoded-font-size' | 'hardcoded-font-family' | 'hardcoded-border-radius' | 'hardcoded-shadow' | 'hardcoded-z-index' | 'magic-number';
  file: string;
  line: number;
  value: string;
...
function scanDirectory(dir: string, exts: string[] = ['.ts', '.tsx', '.js', '.jsx', '.css']): string[] {
  const files: string[] = [];
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (['node_modules', 'build', 'dist', '.next', '__tests__', 'tokens'].includes(entry.name)) continue;
...
function analyzeFile(filePath: string): TokenViolation[] {
  ...
  for (const i = 0; i < lines.length; i++) {
    ...
    for (const match of hexColors) {
      ...
      if (line.includes(`tokens`) || line.includes(`--color-`)) continue;
    ...
    for (const [value, token] of Object.entries(SPACING_TOKENS)) {
      ...
      while ((spacingMatch = regex.exec(line)) !== null) {
        if (line.includes('--spacing-') || line.includes('tokens')) continue;
        violations.push({
...
class EnforceDesignTokensServer extends McpServerBase {
  ...
  private async handleScan(args: unknown) {
    ...
    try {
      ...
      const minSev = sevOrder[severity as keyof typeof sevOrder] ?? 2;

      let violations: TokenViolation[] = [];

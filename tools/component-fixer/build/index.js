...
function autoFixComponent(componentDir, componentName) {
    ...
    if (!content.includes('displayName')) {
        ...
        content = content.replace(/export\s*\{[^}]+\}/, `export { ${componentNameFromExport} }\n${componentNameFromExport}.displayName = "${componentNameFromExport}"`);
    ...
    if (fs.existsSync(testFile) && isVoidElement(componentName)) {
        ...
        if (testContent.includes(`>${componentName}</`) || testContent.includes('>Test Content</') || testContent.includes('>Test</')) {
            ...
            testContent = testContent.replace(/render\(<${componentName}>[^<]*<\/${componentName}>?\)/g, `render(<${componentName} placeholder="test" />)`);
            // Fix assertions that look for text content
            testContent = testContent.replace(/expect\(screen\.getByText\([^)]+\)\)\.toBeInTheDocument\(\)/g, `expect(screen.getByPlaceholderText('test')).toBeInTheDocument()`);
    ...
    if (fs.existsSync(indexFile)) {
        ...
        if (!indexContent.includes('export type') && !indexContent.includes(`${componentName}Props`)) {
            // Add type export
            indexContent = indexContent.replace(/export\s*\{[^}]+\}/, `export { ${componentName} } from './${componentName}'\nexport type { ${componentName}Props } from './${componentName}'`);
...
// MAIN SERVER CLASS
// ============================================================================
class ComponentFixerServer {
    server;
    constructor() {
        this.server = new Server({ name: 'component-fixer', version: '1.0.0' }, { capabilities: { tools: {} } });
    ...
    setupToolHandlers() {
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                {
                    name: 'fix',
                    description: 'Auto-fix React component issues (import paths, dependencies, etc.)',
                    inputSchema: {
                        type: 'object',
                        properties: {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            switch (request.params.name) {
                case 'fix':
                    return await this.handleFix(request.params.arguments);
                default:
    ...
    async run() {
        ...
        console.error('Component Fixer MCP server v1.0 running on stdio');
    }
}
const server = new ComponentFixerServer();

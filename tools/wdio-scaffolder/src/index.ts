...
type ConfigVariant = 'headless-chrome' | 'firefox' | 'parallel' | 'allure' | 'browserstack';
type Feature = 'allure' | 'parallel' | 'docker' | 'browserstack';
...
function parseHtmlForElements(html: string): ParsedElement[] {
...
function bestSelector(el: ParsedElement): string {
...
function elementToPropertyName(el: ParsedElement): string {
...
function generateWdioConf(variant: ConfigVariant, features: Feature[] = []): string {
...
function generatePageObjectClass(pageName: string, elements: ParsedElement[]): string {
...
function generateSpec(feature: string, pageName: string, pomMethods: string[]): string {
  const className = toPascalCase(pageName) + 'Page';
...
function generateExampleSpec(): string {
  return `import { expect } from '@wdio/globals';
...
function generateProjectPackageJson(projectName: string, hasAllure: boolean, hasBS: boolean): string {
...
function generateProjectTsConfig(): string {
  return JSON.stringify(
    {
      compilerOptions: {
        target: 'ES2022',
...
function generateEnvTemplate(hasBS: boolean): string {
...
function generateBrowserStackYml(projectName: string): string {
...
function writeFiles(targetDir: string, files: GeneratedFile[]): string[] {
...
class WdioScaffolderServer extends McpServerBase {
  ...
  private async handleScaffoldProject(args: any): Promise<ToolResult> {
    try {
  ...
  private async handleGeneratePageObject(args: any): Promise<ToolResult> {
    try {
  ...
  private async handleGenerateSpec(args: any): Promise<ToolResult> {
    try {
  ...
  private async handleGenerateConfig(args: any): Promise<ToolResult> {
    try {
      const { variant, outputPath } = args;
      const source = generateWdioConf(variant as ConfigVariant);

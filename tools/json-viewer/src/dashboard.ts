...
function loadAllMeta(limit = 100): SavedResponse[] {
  ensureDir();
  const files = fs.readdirSync(RESPONSES_DIR)
    .filter(f => f.endsWith('.meta.json'));

  const metas: SavedResponse[] = [];
  for (const f of files) {
    try {
      const m = JSON.parse(fs.readFileSync(path.join(RESPONSES_DIR, f), 'utf-8')) as SavedResponse;
      metas.push(m);
...
function watchResponses(): void {
  ensureDir();
  fs.watch(RESPONSES_DIR, (eventType, filename) => {
    if (!filename?.endsWith('.meta.json')) return;
    if (eventType !== 'rename') return; // 'rename' = new file created

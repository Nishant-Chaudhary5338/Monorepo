...
function discoverComponents(rootPath) {
    const components = [];
...
// PHASE EXECUTORS
// ============================================================================
async function reviewComponent(componentPath) {
...
async function fixComponent(componentPath, reviewJson) {
...
function printComponentResult(result, index) {
    ...
    console.log(`  ${index + 1}. ${emoji} ${name} ` +
        `${getGradeEmoji(gradeBefore)} ${gradeBefore} → ${getGradeEmoji(gradeAfter)} ${gradeAfter} ` +
...
async function main() {
    ...
    console.log('');
    const reviewResults = await runInParallel(components, async (path, i) => {
    ...
    const initialReviews = reviewResults.map((r) => {
        if ('error' in r) {
            return { name: basename(r.path || ''), path: r.path || '', error: r.error };
        }
        return {
            name: r.name,
            path: r.path,
    ...
    if (totalFixable > 0) {
        ...
        console.log('');
        const fixResults = await runInParallel(initialReviews.filter(r => r.initialReview), async (comp, i) => {
    ...
    if (componentsWithFixes.length > 0) {
        ...
        console.log('');
        const reReviewResults = await runInParallel(componentsWithFixes, async (comp, i) => {

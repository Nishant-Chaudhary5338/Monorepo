...
import { sortTopLeft, isInside, overlaps, aspectRatio, bboxArea, isHorizontallyAligned } from '../utils/bbox.js';
import { groupByRow } from '../utils/grouping.js';
...
export function detectComponents(
  ...
  for (const text of texts) {
    ...
    const detection = createDetection(
      ...
      classified.reason,
      text.fontSize === 'heading' || classified.type === 'Heading',
      imageHeight,
    );
  ...
  for (const block of blocks) {
    ...
    if (block.type === 'text' && isInputShape(block.bbox, block.color)) {
      ...
      const detection = createDetection(
        ...
        label,
        0.7,
        `Shape matches input field pattern, label: "${label}"`,
        false,
        imageHeight,
        block.color,
      );
...
function createDetection(
  ...
  if (bbox.width < 10 || bbox.height < 8) return null;

  // Skip impossibly large detections (full image)
  if (bboxArea(bbox) > 500000) return null;
...
function classifyByShape(
  ...
  imageHeight: number,
): { type: ComponentType; confidence: number; reason: string } | null {
...
function findNearbyText(
  ...
  let bestMatch: ExtractedText | null = null;
  ...
  for (const text of texts) {
    ...
    switch (direction) {
      case 'above':
        if (text.bbox.y >= bbox.y) continue;
      ...
      case 'below':
        ...
        break;
      case 'left':
        if (text.bbox.x >= bbox.x) continue;
 ...
 */
function associateLabels(detections: RawDetection[], texts: ExtractedText[]): void {
...
function detectContainers(
  ...
  const containers: RawDetection[] = [];
  const containerIndex = 0;

  for (const block of blocks) {
    if (block.type !== 'region') continue;
    ...
    if (area > 20000 && area < imageWidth * imageHeight * 0.8) {
      ...
      const isCard =
        ratio > 0.5 && ratio < 3 && block.bbox.width > 100 && block.bbox.height > 80;
 ...
 */
function deduplicateDetections(detections: RawDetection[]): RawDetection[] {

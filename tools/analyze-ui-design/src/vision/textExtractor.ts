 ...
 */
export async function extractText(image: ParsedImage): Promise<ExtractedText[]> {
  try {
    ...
    if (result.data.words) {
      for (const word of result.data.words) {
        ...
        if (word.confidence < 30) continue; // Skip low-confidence detections
...
export async function extractTextFromRegion(
  ...
  try {
    const regionBuffer = await extractRegion(
      image,
      bbox.x,
      bbox.y,
      bbox.width,
 ...
 */
function classifyFontSize(
  textHeight: number,
  imageHeight: number,
): 'small' | 'medium' | 'large' | 'heading' {
  const relativeHeight = textHeight / imageHeight;

  if (relativeHeight > 0.05) return 'heading';
  if (relativeHeight > 0.03) return 'large';
  if (relativeHeight > 0.015) return 'medium';
  return 'small';
}
 ...
 */
function classifyFontWeight(word: unknown): 'normal' | 'medium' | 'bold' {
 ...
 */
function mergeNearbyTexts(texts: ExtractedText[]): ExtractedText[] {
  ...
  for (const i = 0; i < sorted.length; i++) {
    ...
    const group: ExtractedText[] = [sorted[i]];
    used.add(i);
    ...
    for (const j = i + 1; j < sorted.length; j++) {
      ...
      if (yDiff > avgHeight * 0.5) continue; // Not on the same line

      // Check horizontal proximity
      const xGap = curr.bbox.x - (prev.bbox.x + prev.bbox.width);
      if (xGap > avgHeight * 3) continue; // Too far apart
 ...
 */
function extractLineTexts(data: unknown, startIndex: number): ExtractedText[] {
  const lineTexts: ExtractedText[] = [];
 ...
 */
export async function quickExtract(image: ParsedImage): Promise<string> {
  try {
    const result = await Tesseract.recognize(image.buffer, 'eng', {
      logger: () => {},
    });

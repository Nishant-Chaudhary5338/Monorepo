 ...
 */
function sortByPosition(components: DetectedComponent[]): DetectedComponent[] {
  return [...components].sort((a, b) => {
    const aBbox = a.position.bbox;
    const bBbox = b.position.bbox;
    if (Math.abs(aBbox.y - bBbox.y) < 15) return aBbox.x - bBbox.x;
 ...
 */
function groupByRow(components: DetectedComponent[], tolerance = 20): DetectedComponent[][] {
  if (components.length === 0) return [];
  const sorted = [...components].sort((a, b) => a.position.bbox.y - b.position.bbox.y);
  const groups: DetectedComponent[][] = [];
  let currentGroup: DetectedComponent[] = [sorted[0]];
 ...
 */
function isVerticalStack(components: DetectedComponent[]): boolean {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    if (Math.abs(centerX - firstCenterX) <= maxDeviation) alignedCount++;
  }
...
export function buildLayout(
  ...
  imageWidth: number,
  imageHeight: number,
): { layout: LayoutStructure; sectionMap: Map<string, string> } {
 ...
 */
function detectLayoutType(components: DetectedComponent[], imageHeight: number): LayoutType {
...
function buildSections(
  ...
  const headerComponents = components.filter(
    c => c.position.bbox.y + c.position.bbox.height < imageHeight * 0.15,
  );
  ...
  const footerComponents = components.filter(
    c => c.position.bbox.y > imageHeight * 0.9,
  );
  ...
  const navCandidates = components.filter(
    c => !assigned.has(c.id) && (c.name === 'Link' || c.name === 'Button' || c.name === 'Nav'),
  );
  ...
  const containers = unassignedComponents.filter(
    c => !assigned.has(c.id) && c.name === 'Container',
  );

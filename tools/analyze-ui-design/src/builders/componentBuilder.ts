...
function normalizeDetection(
  ...
  if (raw.text) props.text = raw.text;
  if (raw.label) props.label = raw.label;
  ...
  switch (raw.inferredType) {
    ...
    case 'Heading': {
      const text = raw.text || '';
      variant = inferHeadingVariant(
        raw.props?.fontSize as 'small' | 'medium' | 'large' | 'heading' || 'large',
        raw.bbox.y,
        imageHeight,
      );
      props.text = text;
...
function generatePlaceholder(label: string): string {
  ...
  if (/email/.test(lower)) return 'Enter your email';
  if (/password/.test(lower)) return 'Enter your password';
  if (/username/.test(lower)) return 'Enter your username';
  if (/name/.test(lower)) return 'Enter your name';
  if (/phone/.test(lower)) return 'Enter your phone number';
  if (/search/.test(lower)) return 'Search...';
  if (/address/.test(lower)) return 'Enter your address';
 ...
 */
function assignHierarchy(components: DetectedComponent[]): void {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    if (container.name !== 'Container') continue;

    for (let j = 0; j < components.length; j++) {
      const child = components[j];
      if (child.id === container.id) continue;
      if (child.parentId) continue; // Already assigned

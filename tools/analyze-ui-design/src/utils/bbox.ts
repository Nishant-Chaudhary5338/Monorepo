 ...
 */
export function bboxBottomRight(bbox: BoundingBox): Point {
  return {
    x: bbox.x + bbox.width,
 ...
 */
export function isInside(inner: BoundingBox, outer: BoundingBox): boolean {
  return (
    inner.x >= outer.x &&
    inner.y >= outer.y &&
 ...
 */
export function isHorizontallyAligned(a: BoundingBox, b: BoundingBox, tolerance = 0.2): boolean {
 ...
 */
export function isVerticallyAligned(a: BoundingBox, b: BoundingBox, tolerance = 0.2): boolean {
  const centerA = bboxCenter(a);
  const centerB = bboxCenter(b);
 ...
 */
export function sortTopLeft<T extends { bbox: BoundingBox }>(items: T[]): T[] {
  return [...items].sort((a, b) => {
    const yDiff = a.bbox.y - b.bbox.y;
    if (Math.abs(yDiff) < 10) return a.bbox.x - b.bbox.x;
    return yDiff;
  });
}
 ...
 */
export function sortByPosition<T extends { bbox: BoundingBox }>(items: T[]): T[] {
  return [...items].sort((a, b) => {
 ...
 */
export function aspectRatio(bbox: BoundingBox): number {
 ...
 */
export function isSquare(bbox: BoundingBox, tolerance = 0.2): boolean {
 ...
 */
export function isWide(bbox: BoundingBox, threshold = 2): boolean {
 ...
 */
export function isTall(bbox: BoundingBox, threshold = 2): boolean {

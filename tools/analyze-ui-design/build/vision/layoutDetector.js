 ...
 */
export async function detectLayout(image) {
    const blocks = [];
    ...
    for (let i = 0; i < mergedRegions.length; i++) {
        ...
        if (region.width < 30 || region.height < 20)
            continue;
        // Skip regions that cover most of the image (background)
        if (region.width > image.width * 0.95 && region.height > image.height * 0.95)
            continue;
...
async function detectEdges(image) {
    ...
    for (let y = 1; y < height - 1; y++) {
        for (let x = 1; x < width - 1; x++) {
            ...
            const diffRight = Math.abs(buffer[idx] - buffer[rightIdx]) +
                Math.abs(buffer[idx + 1] - buffer[rightIdx + 1]) +
                Math.abs(buffer[idx + 2] - buffer[rightIdx + 2]);
            // Compare with bottom neighbor
            const diffBottom = Math.abs(buffer[idx] - buffer[bottomIdx]) +
...
function classifyBlockType(region, image) {
    ...
    if (ratio > 0.7 && ratio < 1.3 && region.width < 150)
        return 'image';
    return 'shape';
}
/**
 ...
 */
function calculateRegionConfidence(region, image) {
    const confidence = 0.5;
    // Prefer regions with reasonable sizes
    if (region.width > 50 && region.height > 30)
        confidence += 0.1;
    if (region.width < image.width * 0.8 && region.height < image.height * 0.8)
        confidence += 0.1;
    // Prefer regions that aren't too small
    if (region.width * region.height > 5000)
        confidence += 0.1;
    // Prefer regions in typical UI positions (not edges)
    if (region.x > 10 && region.y > 10)
        confidence += 0.1;

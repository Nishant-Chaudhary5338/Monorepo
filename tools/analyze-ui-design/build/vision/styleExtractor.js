 ...
 */
export async function extractStyles(image) {
 ...
 */
async function extractGlobalColors(image) {
    try {
...
async function extractColorPalette(image) {
    ...
    if (sortedByBrightness.length > 1) {
        palette.background = rgbToHex(sortedByBrightness[sortedByBrightness.length - 1].r, sortedByBrightness[sortedByBrightness.length - 1].g, sortedByBrightness[sortedByBrightness.length - 1].b);
    }
 ...
 */
function analyzeTypography(_image) {
    ...
    for (const col of sampleColumns) {
        ...
        for (const y = 0; y < height; y++) {
            ...
            if (isLight) {
                ...
                if (rowGap > 5)
                    verticalGaps.push(rowGap);
                rowGap = 0;
            }
        }
    }
    // Analyze gap consistency
    let gapPattern = 'mixed';
 ...
 */
export async function extractRegionColors(image, x, y, width, height) {
    try {
        ...
        return {
            dominant: rgbToHex(dominant.r, dominant.g, dominant.b),
            average: rgbToHex(Math.round(stats.channels[0].mean), Math.round(stats.channels[1].mean), Math.round(stats.channels[2].mean)),
            hasBorder: false,
        };
    }
    catch {

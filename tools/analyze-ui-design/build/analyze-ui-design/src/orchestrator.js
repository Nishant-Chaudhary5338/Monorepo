 ...
 */
export async function analyzeUI(filePath, options = {}) {
    ...
    const blueprint = buildBlueprint(finalMeta, allComponents, layout, styles.palette, styles.typography, styles.spacing, opts);
    return blueprint;
}
/**
 * Parse input file into images
 */
async function parseInput(filePath, options) {
    ...
    throw new Error(`Unsupported file format: ${path.extname(filePath)}. Supported: .png, .jpg, .jpeg, .webp, .gif, .bmp, .tiff, .pdf`);
}
/**
 * Analyze a single image through the full vision pipeline
 */
async function analyzeImage(image, sourcePath, pageNumber, totalPages, options) {
    ...
    const components = buildComponents(filteredDetections, texts, image.height, sectionMap, pageNumber);
    // Step 5: Build final blueprint
    const source = isPdfFile(sourcePath) ? 'pdf' : 'image';
    const meta = {
        source,
    ...
    return buildBlueprint(meta, components, layout, styles.palette, styles.typography, styles.spacing, options);
}

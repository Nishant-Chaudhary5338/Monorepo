 ...
 */
export function buildBlueprint(meta, components, layout, palette, typography, spacing, options) {
 ...
 */
function buildDesignSystemHints(components, palette, typography, spacing) {
 ...
 */
function buildTestingHints(components, layout) {
    ...
    if (formSection) {
        const formComponents = formSection.componentIds
            ...
            .filter(Boolean);
        const inputsAndButtons = formComponents.filter(c => c.name === 'Input' || c.name === 'Button');

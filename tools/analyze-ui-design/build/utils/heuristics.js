import { bboxArea, aspectRatio } from './bbox.js';
import { groupByRow, isVerticalStack } from './grouping.js';
// ============================================================================
 ...
 */
export function isButtonShape(bbox, color) {
    ...
    if (bbox.height < 20 || bbox.height > 80)
        return false;
    if (bbox.width < 50 || bbox.width > 500)
        return false;
    // Buttons often have rounded corners or colored backgrounds
    if (color) {
        if (color.borderRadius && color.borderRadius !== 'none')
            return true;
        if (color.hasBorder)
            return true;
    }
 ...
 */
export function isInputShape(bbox, color) {
    const ratio = aspectRatio(bbox);
    // Inputs are typically wide and not very tall
    if (ratio < 2)
        return false;
    if (bbox.height < 25 || bbox.height > 70)
        return false;
    if (bbox.width < 100)
        return false;
 ...
 */
export function isContainerShape(bbox) {
 ...
 */
export function isHeadingShape(bbox, imageHeight) {
    // Headings are typically in the upper portion of the screen
    if (bbox.y > imageHeight * 0.5)
        return false;
    return true;
}
// ============================================================================
 ...
 */
export function classifyByFontSize(fontSize, text) {
    if (fontSize === 'heading')
        return 'Heading';
    if (fontSize === 'large') {
 ...
 */
export function inferSectionType(components, imageHeight) {
    ...
    if (avgY < imageHeight * 0.15)
        return 'header';
    // Footer: components in the bottom 15% of the screen
    if (maxY > imageHeight * 0.85)
        return 'footer';
 ...
 */
export function detectFormPattern(detections) {
 ...
 */
export function detectNavigationPattern(detections) {
    const navCandidates = detections.filter(d => d.inferredType === 'Link' || d.inferredType === 'Button' || d.inferredType === 'Nav');
 ...
 */
export function detectInputVariant(label) {
    const lower = label.toLowerCase();
    if (/email|e-mail/.test(lower))
        return 'email';
    if (/password|pwd|pass/.test(lower))
        return 'password';
    if (/search/.test(lower))
        return 'search';
    if (/phone|tel|mobile/.test(lower))
        return 'tel';
    if (/url|website|link/.test(lower))
        return 'url';
    if (/amount|price|number|qty|quantity/.test(lower))
        return 'number';
    if (/date|dob|birth/.test(lower))
        return 'text';
    return 'text';
}
// ============================================================================
 ...
 */
export function inferValidationRules(label) {
    const rules = [];
    const lower = label.toLowerCase();
    // Required fields
 ...
 */
export function detectScreenType(detections, formDetected, navDetected) {
 ...
 */
export function inferButtonVariant(text, color) {
    const lower = text.toLowerCase();
    if (/cancel|close|back|dismiss/.test(lower))
        return 'outline';
    if (/delete|remove|destroy/.test(lower))
        return 'destructive';
    if (/learn\s*more|read\s*more|view|see/.test(lower))
        return 'link';
    if (/forgot|reset/.test(lower))
        return 'ghost';
    if (color?.borderRadius === 'full')
        return 'secondary';
    return 'primary';
}
/**
 ...
 */
export function inferHeadingVariant(fontSize, yPosition, imageHeight) {
    const relativeY = yPosition / imageHeight;
    if (fontSize === 'heading' && relativeY < 0.1)
        return 'h1';
    if (fontSize === 'heading')
        return 'h2';
    if (fontSize === 'large' && relativeY < 0.2)
        return 'h2';
    if (fontSize === 'large')
        return 'h3';
    return 'h4';

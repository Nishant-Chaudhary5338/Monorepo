 ...
 */
export function groupByRow(items, tolerance = 20) {
    if (items.length === 0)
        return [];
 ...
 */
export function groupByColumn(items, tolerance = 20) {
    if (items.length === 0)
        return [];
 ...
 */
export function groupByProximity(items, maxDistance = 100) {
    if (items.length === 0)
        return [];
 ...
 */
export function groupByHorizontalAlignment(items, tolerance = 0.15) {
    if (items.length === 0)
        return [];
    const groups = [];
 ...
 */
export function groupByVerticalAlignment(items, tolerance = 0.15) {
    if (items.length === 0)
        return [];
    const groups = [];
    const assigned = new Set();
 ...
 */
export function isVerticalStack(items) {
    if (items.length < 2)
        return false;
    const sorted = [...items].sort((a, b) => a.bbox.y - b.bbox.y);
 ...
 */
export function isHorizontalRow(items) {
    if (items.length < 2)
        return false;
    const sorted = [...items].sort((a, b) => a.bbox.x - b.bbox.x);
    const firstCenter = bboxCenter(sorted[0].bbox);
 ...
 */
export function getConsistentSpacing(items, direction, tolerance = 0.3) {
    if (items.length < 2)
        return { consistent: true, averageGap: 0 };
 ...
 */
export function mergeOverlappingGroups(groups) {
    const merged = [];
 ...
 */
export function sortGroupsByPosition(groups) {

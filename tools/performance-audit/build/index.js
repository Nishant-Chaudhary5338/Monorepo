...
// MAIN SERVER
// ============================================================================
class PerformanceAuditServer {
    server;
    constructor() {
        this.server = new Server({ name: 'performance-audit', version: '1.0.0' }, { capabilities: { tools: {} } });
    ...
    setupToolHandlers() {
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                {
                    name: 'audit_bundle_size',
                    description: 'Analyze code for heavy imports, large dependencies, and bundle size issues',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            path: { type: 'string', description: 'Project root or file path to audit' },
                        },
                        required: ['path'],
                    },
                },
                {
                    name: 'detect_heavy_imports',
                    description: 'Find imports from heavy libraries (moment, lodash, jquery, etc.) that increase bundle size',
                    inputSchema: {
                        type: 'object',
                        properties: {
                ...
                },
                {
                    ...
                    description: 'Detect patterns that impact render performance: sync ops, memory leaks, deep nesting',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            path: { type: 'string', description: 'Path to scan' },
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            switch (request.params.name) {
                case 'audit_bundle_size':
                    return await this.handleAuditBundle(request.params.arguments);
                case 'detect_heavy_imports':
                    return await this.handleDetectHeavy(request.params.arguments);
                case 'analyze_render_performance':
                    return await this.handleRenderPerf(request.params.arguments);
                default:
                    throw new McpError(ErrorCode.MethodNotFound, `Unknown tool: ${request.params.name}`);
            }
        });
    }
    async handleAuditBundle(args) {
        ...
        try {
            ...
            const allIssues = analyzePackageJson(projectRoot);
    ...
    async run() {
        ...
        console.error('Performance Audit MCP server running on stdio');
    }
}

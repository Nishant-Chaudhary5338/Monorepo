...
function runLighthouse(url: string, outputPath: string, categories: string[] = ['performance', 'accessibility', 'best-practices', 'seo']): LighthouseResult {
  ...
  const auditList = Object.entries(audits)
    .filter(([_, a]: unknown) => a.score !== null && a.score < 1)
    .map(([key, a]: unknown) => ({
      name: key,
...
function analyzeHtmlFile(filePath: string): { score: number; issues: string[] } {
      ...
      if (isDir) {
        const scan = (dir: string) => {
          for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
            ...
            if (entry.isDirectory() && !['node_modules', 'build', 'dist'].includes(entry.name)) scan(full);

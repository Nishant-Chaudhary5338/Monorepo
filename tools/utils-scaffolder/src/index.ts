...
import { writeModule, ensureDir } from './utils/file-writer.js';
import { validateModuleName, ALL_MODULES } from './utils/name-validator.js';
...
class UtilsScaffolderServer extends McpServerBase {
  ...
  protected registerTools(): void {
    ...
    this.addTool(
      'generate_module',
      'Generate a specific utility module (e.g., api, validation, hooks, string, array)',
      {
        type: 'object',
        properties: {
          moduleName: {
    ...
    this.addTool(
      ...
      'Generate ALL 21 modules to create the complete @repo/utils package',
      {
        type: 'object',
        properties: {
          outputPath: {
    ...
    this.addTool(
      'generate_hook',
      'Generate a specific React hook within the hooks module',
      {
        type: 'object',
        properties: {
          hookName: {
    ...
    this.addTool(
      'generate_utility',
      'Generate a specific utility function within a module',
      {
        type: 'object',
        properties: {
          utilityName: {
    ...
    this.addTool(
      'validate_package',
      'Validate the @repo/utils package: TypeScript compilation, barrel exports, tests',
      {
        type: 'object',
        properties: {
          packagePath: {
  ...
  private async handleGenerateModule(args: unknown): Promise<ToolResult> {
    const { moduleName, outputPath = DEFAULT_OUTPUT } = args as {
      moduleName: string;
      outputPath?: string;
    };
    ...
    if (!generator) {
      return this.error(new Error(
        `Unknown module: "${moduleName}". Available modules: ${ALL_MODULE_NAMES.join(', ')}`
      ));
    }
  ...
  private async handleGenerateAllModules(args: unknown): Promise<ToolResult> {
    const { outputPath = DEFAULT_OUTPUT } = args as { outputPath?: string };

    const results: { module: string; files: number; path: string }[] = [];
  ...
  private async handleGenerateHook(args: unknown): Promise<ToolResult> {
    const { hookName, outputPath = DEFAULT_OUTPUT } = args as {
      hookName: string;
      outputPath?: string;
    };
  ...
  private async handleGenerateUtility(args: unknown): Promise<ToolResult> {
    const { utilityName, moduleName, outputPath = DEFAULT_OUTPUT } = args as {
      utilityName: string;
      moduleName: string;
      outputPath?: string;
    };
    ...
    if (!ALL_MODULE_NAMES.includes(moduleName)) {
      return this.error(new Error(
        `Unknown module: "${moduleName}". Available: ${ALL_MODULE_NAMES.join(', ')}`
      ));
    }
  ...
  private async handleValidatePackage(args: unknown): Promise<ToolResult> {
    const { packagePath = path.resolve(__dirname, '../../../../packages/utils') } = args as {
      packagePath?: string;
    };
...
function generateBarrelExport(modules: string[]): string {
...
function generateHookCode(hookName: string): string {
...
function generateHookTestCode(hookName: string): string {
...
function generateUtilityCode(utilityName: string, moduleName: string): string {
  return `// ============================================================================
// ${utilityName} Utility
// ============================================================================
...
function generateUtilityTestCode(utilityName: string): string {
  return `import { describe, it, expect } from 'vitest'

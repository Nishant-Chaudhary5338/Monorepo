...
import type { Violation, FileScanResult, ScanOptions, RuleName } from './types.js';
...
interface FixPattern {
  name: string;
  rule: RuleName;
  detect: (violation: Violation, line: string) => boolean;
  fix: (line: string, violation: Violation) => string;
}
...
const FIX_PATTERNS: FixPattern[] = [
  ...
  // modifiers: add `as const` to arrays
  {
    ...
    fix: (line) => {
      if (line.includes('as const')) return line;
...
function applyFix(line: string, violation: Violation): { fixed: string; status: 'fixed' | 'skipped' | 'manual'; reason?: string } {
...
function fixFile(fileResult: FileScanResult, options: BatchFixOptions): FixResult[] {
...
function parseArgs(args: string[]): BatchFixOptions {
  const options: BatchFixOptions = {
    path: '',
    dryRun: false,
    backup: true,
  };
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case '--rule':
      case '-r':
        options.rules = (args[++i] || '').split(',') as RuleName[];
        break;
      case '--severity':
      case '-s':
...
function printReport(report: BatchFixReport, dryRun: boolean) {
  console.log('\n' + '='.repeat(60));
...
async function main() {
  ...
  console.log(`\n🔍 Scanning: ${targetPath}`);
  if (options.dryRun) console.log('  Mode: DRY RUN (no files will be modified)');
  if (options.rules) console.log(`  Rules: ${options.rules.join(', ')}`);

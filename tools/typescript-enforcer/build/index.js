...
// SERVER
// ============================================================================
const server = new Server({ name: 'typescript-enforcer', version: '2.0.0' }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({ tools: TOOLS }));
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    const { name, arguments: args } = request.params;
    if (!args && name !== 'list_rules')
    ...
    try {
        ...
        switch (name) {
            ...
            case 'scan_specific_rule': {
                const { rule, path: targetPath, severity } = args;
                if (!VALID_RULES.includes(rule))
                    throw new Error(`Invalid rule: ${rule}`);
                const resolvedPath = path.resolve(targetPath);
                if (!fs.existsSync(resolvedPath))
                    throw new Error(`Path not found: ${resolvedPath}`);
                const options = { rules: [rule], severity: severity || 'info' };

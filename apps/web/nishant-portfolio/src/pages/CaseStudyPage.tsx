...
const CaseStudyPage = () => {
  ...
  return (
    <div style={{ background: "var(--bg-primary)", color: "var(--text-primary)", minHeight: "100vh" }}>
      ...
      <div style={{ maxWidth: "740px", margin: "0 auto", padding: "4rem clamp(1.25rem, 5vw, 2rem) 2rem" }}>
        ...
        >
          {[
            ...
            { label: "Period", value: meta.years },
            { label: "Status", value: meta.status },
          ].map(({ label, value }) => (
            <div key={label} style={{ padding: "1rem 0 1rem", borderRight: "1px solid var(--rule)", paddingRight: "1.5rem", paddingLeft: "0.5rem" }}>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.68rem", textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text-muted)", marginBottom: "0.4rem" }}>
                {label}
              </div>
          ...
          <div style={{ padding: "1rem 0.5rem 1rem" }}>
            ...
            <div style={{ fontFamily: "var(--font-mono)", fontSize: "0.78rem", color: "var(--text-primary)", lineHeight: 1.5 }}>
              {meta.stack}
            </div>
          </div>
        </div>
      ...
      {/* Reuse same article-body styles from ArticlePage */}
      <style>{`
        .article-body { font-family: var(--font-body); font-size: 1.05rem; line-height: 1.75; color: var(--text-secondary); }
        .article-body h1 { display: none; }
        .article-body h2 { font-family: var(--font-display); font-weight: 400; font-size: clamp(1.6rem, 3vw, 2.2rem); line-height: 1.15; letter-spacing: -0.02em; color: var(--text-primary); margin: 3.5rem 0 1rem; padding-top: 2rem; border-top: 1px solid var(--rule); }
        .article-body h3 { font-family: var(--font-display); font-weight: 400; font-size: 1.35rem; color: var(--text-primary); margin: 2.5rem 0 0.8rem; letter-spacing: -0.01em; }
        .article-body p { margin: 0 0 1.4rem; }
        .article-body strong { color: var(--text-primary); font-weight: 600; }
        .article-body a { color: var(--accent-warm); text-underline-offset: 3px; }
        .article-body ul, .article-body ol { margin: 0 0 1.4rem 1.5rem; display: flex; flex-direction: column; gap: 0.4rem; }
        .article-body li { line-height: 1.65; }
        .article-body blockquote { border-left: 3px solid var(--accent-warm); padding: 0.8rem 0 0.8rem 1.5rem; margin: 1.8rem 0; color: var(--text-muted); font-style: italic; }
        .article-body blockquote p { margin: 0; }
        .article-body code:not(pre code) { font-family: var(--font-mono); font-size: 0.88em; background: var(--bg-secondary); padding: 0.15em 0.4em; border-radius: 3px; color: var(--accent-warm); }
        .article-body pre { background: var(--bg-secondary) !important; border: 1px solid var(--border-color); border-radius: 0; padding: 1.4rem 1.6rem; overflow-x: auto; margin: 1.8rem 0; font-family: var(--font-mono); font-size: 0.84rem; line-height: 1.65; }
        .article-body pre code { background: none !important; padding: 0; color: var(--text-primary); font-size: inherit; }
        .article-body table { width: 100%; border-collapse: collapse; margin: 1.8rem 0; font-family: var(--font-mono); font-size: 0.84rem; }
        .article-body th { text-align: left; padding: 0.6rem 1rem; border-bottom: 2px solid var(--rule); color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.08em; font-size: 0.72rem; font-weight: 400; }
        .article-body td { padding: 0.65rem 1rem; border-bottom: 1px solid var(--rule); color: var(--text-secondary); vertical-align: top; }
        .article-body hr { border: none; border-top: 1px solid var(--rule); margin: 3rem 0; }
        .dark .article-body pre { background: #111113 !important; }

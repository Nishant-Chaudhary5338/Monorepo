...
export default function LeadTimeChart({ data }: Props) {
  ...
  return (
    <div className="widget-card">
      <h3 className="mb-1 text-lg font-semibold text-foreground">Avg Lead Time</h3>
      <p className="mb-4 text-sm text-muted-foreground">Seconds per annotation (project avg: {PROJECT_AVG_LEAD_TIME}s)</p>
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={chartData} layout="vertical" margin={{ left: 20, right: 20, top: 5, bottom: 5 }}>
          ...
          <Bar dataKey="leadTime" radius={[0, 6, 6, 0]}>
            {chartData.map((entry) => (
              <Cell key={entry.name} fill={entry.diff > 5 ? "hsl(var(--chart-1))" : "hsl(var(--chart-3))"} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
